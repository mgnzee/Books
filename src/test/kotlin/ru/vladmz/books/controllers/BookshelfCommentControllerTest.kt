...
@WebMvcTest(controllers = [BookshelfCommentController::class])
@AutoConfigureMockMvc(addFilters = false)
class BookshelfCommentControllerTest(@Autowired val mockMvc: MockMvc) {
    ...
    fun setUp() {
        ...
        comment.targetId = bookshelfId
        comment.targetType = TargetType.BOOKSHELF
    ...
    fun findByTargetId(){
        ...
        mockMvc.get("/bookshelves/$bookshelfId/comments") {
        ...
        verify(commentService).getCommentsByTargetId(
            argThat { it.type() == TargetType.BOOKSHELF && it.id() == bookshelfId },
            any()
        )
    }
    ...
    fun findById(){
        ...
        mockMvc.get("/bookshelves/$bookshelfId/comments/$commentId") {
            contentType = MediaType.APPLICATION_JSON
        }.andExpect {
            status { isOk() }
    ...
    fun findById_shouldReturn404(){
        ...
        mockMvc.get("/bookshelves/$bookshelfId/comments/$wrongId") {
            contentType = MediaType.APPLICATION_JSON
        }.andExpect {
            status { isNotFound() }
    ...
    fun findReplies(){
        ...
        mockMvc.get("/bookshelves/$bookshelfId/comments/$commentId/replies") {
            contentType = MediaType.APPLICATION_JSON
        }.andExpect {
            status { isOk() }
            jsonPath("$.content[0].id") { value(commentId) }
    ...
    fun findReplies_shouldReturn404(){
        ...
        mockMvc.get("/bookshelves/$bookshelfId/comments/$wrongId/replies") {
            contentType = MediaType.APPLICATION_JSON
        }.andExpect {
            status { isNotFound() }
            content { contentType(MediaType.APPLICATION_JSON) }
    ...
    fun updateComment_shouldReturn404(){
        ...
        mockMvc.patch("/bookshelves/$bookshelfId/comments/$wrongId") {
            contentType = MediaType.APPLICATION_JSON
            content = objectMapper.writeValueAsString(request)
            accept = MediaType.APPLICATION_JSON
        }.andExpect {
    ...
    fun deleteComment(){
        ...
        verify(commentService).deleteComment(commentId, CommentTarget.ofBookshelf(bookshelfId))
    }
    ...
    fun deleteComment_shouldReturn404(){
        ...
        mockMvc.delete("/bookshelves/$bookshelfId/comments/$wrongId") {
            accept = MediaType.APPLICATION_JSON
        }.andExpect {
            status { isNotFound() }
            jsonPath("$.status") { value(404) }
