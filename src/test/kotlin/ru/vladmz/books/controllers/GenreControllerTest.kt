...
class GenreControllerTest (@Autowired val mockMvc: MockMvc) {
    ...
    fun findBooksByGenreId(){
        ...
        whenever(genreService.findBooksByGenre(genreId)).thenReturn(books)
        ...
        mockMvc.get("/genres/$genreId/books"){
            ...
            jsonPath("$.length()") { value(2) }
            jsonPath("$[0].id") { value(1) }
            jsonPath("$[0].title") { value("Crime and punishment") }
            jsonPath("$[1].id") { value(2) }
            jsonPath("$[1].title") { value("Le Comte de Monte-Cristo") }
        }
    }
    ...
    fun findBooksByGenreId_shouldReturn404() {
        val genreId = 100
        whenever(genreService.findBooksByGenre(genreId)).thenThrow(GenreNotFoundException(genreId))
